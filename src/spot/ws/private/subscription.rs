...
pub struct PrivateSubscribeParams {
    pub subscription_topics: Vec<PrivateSubscriptionTopic>,
...
pub enum PrivateSubscriptionTopic {
    AccountUpdate,
    AccountDeals,
    AccountOrders,
}

impl PrivateSubscriptionTopic {
    pub fn to_subscription_param(&self) -> String {
        match self {
            PrivateSubscriptionTopic::AccountUpdate => "spot@private.account.v3.api".to_string(),
            PrivateSubscriptionTopic::AccountDeals => "spot@private.deals.v3.api".to_string(),
            PrivateSubscriptionTopic::AccountOrders => "spot@private.orders.v3.api".to_string(),
        }
    }
}
...
impl PrivateSubscribe for MexcSpotPrivateWsClient {
    async fn private_subscribe(&self, params: PrivateSubscribeParams) -> Result<PrivateSubscribeOutput, PrivateSubscribeError> {
        ...
        if subscription_topics.is_empty() {
            ...
            return Ok(PrivateSubscribeOutput {});
        }
...
pub struct PrivateUnsubscribeParams {
    pub subscription_topics: Vec<PrivateSubscriptionTopic>,
...
#[derive(Debug)]
pub struct PrivateUnsubscribeOutput {}

#[derive(Debug, thiserror::Error)]
pub enum PrivateUnsubscribeError {
    #[error("Failed to serialize payload to JSON: {0}")]
    SerdeJsonError(#[from] serde_json::Error),
    #[error("Failed to acquire websocket: {0}")]
    AcquireWebsocketError(#[from] AcquireWebsocketError),
...
#[async_trait]
pub trait PrivateUnsubscribe {
    async fn private_unsubscribe(&self, params: PrivateUnsubscribeParams) -> Result<PrivateUnsubscribeOutput, PrivateUnsubscribeError>;
}

#[async_trait]
impl PrivateUnsubscribe for MexcSpotPrivateWsClient {
    async fn private_unsubscribe(&self, params: PrivateUnsubscribeParams) -> Result<PrivateUnsubscribeOutput, PrivateUnsubscribeError> {
        ...
        if subscription_topics_to_remove.is_empty() {
            ...
            return Ok(PrivateUnsubscribeOutput {});
        }
